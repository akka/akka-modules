...
class HawtDispatcher(val aggregate: Boolean = true, val parent: DispatchQueue = globalQueue) extends MessageDispatcher  {
  ...
  private[akka] def executeFuture(invocation: FutureInvocation){
    parent {
      invocation.run
    }
  }
...
/**
 * Use this in your akka.conf to create and configure a HawtDispatcher
 * Only parameter is "aggregate" which is a boolean (on/off) it defaults to true
 */

...
/**
 * A persistent actor based on Mongo sortedset storage..
 * Based on the Redis test for the same
 * <p/>
...
class SortedSetActor extends Actor {
  ...
  def atomicReceive: Receive = {
    ...
    case SCORE(h) =>
      self.reply(hackers.zscore(h.name))
...
import MongoNativeSortedSetStorageBackend._

@RunWith(classOf[JUnitRunner])
class MongoPersistentSortedSetTestIntegration extends
  Spec with
  ShouldMatchers with
  BeforeAndAfterAll {
